    ...
    <section className="w-full py-10 md:py-16 lg:py-20 bg-background/50 relative overflow-hidden">
      ...
      <div className="container px-4 md:px-6 mx-auto">
        ...
        >
          ...
          >
            <span className="rounded-full px-4 py-1 text-sm font-medium bg-primary/10 text-primary animate-shimmer bg-[length:200%_100%] bg-[linear-gradient(110deg,transparent_25%,rgba(var(--primary-rgb),0.1)_50%,transparent_75%)]">Ce que disent mes clients</span>
          </motion.div>
          
          <div className="space-y-2">
            <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl">Témoignages</h2>
          ...
          >
            {[...Array(5)].map((_, i) => (
              ...
              >
                <FaStar className="text-yellow-400 h-4 w-4 md:h-5 md:w-5" />
              </motion.div>
            ))}
        ...
        >
          ...
          <motion.p 
            className="text-muted-foreground text-xs md:text-sm max-w-md mx-auto"
...
function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  return (
    <motion.div
      className="h-full rounded-xl border bg-background/80 backdrop-blur-sm p-4 md:p-5 shadow-md hover:shadow-glow transition-all duration-300"
      initial={{ opacity: 0, y: 20 }}
    ...
    >
      <div className="mb-2 flex justify-between items-start">
        <motion.div 
          className="h-7 w-7 md:h-8 md:w-8 rounded-full flex items-center justify-center bg-primary/10 text-primary"
